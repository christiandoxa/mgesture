...
Standalone distribution currently has three publishable matrix rows: `x86_64-unknown-linux-gnu`, `aarch64-apple-darwin`, and `x86_64-pc-windows-msvc`. Each is packaged as a PyInstaller onedir archive with the pinned model and Python reference engine. The Linux bundle's current native-library audit reaches GLIBC 2.38; this is the Linux matrix baseline, not a claim for older distributions. Matching-host CI extracts each archive and passes `--version`, `self-test --headless --fake-input`, and runtime diagnostics without system Python or Mojo. Camera, Accessibility, and real-pointer behavior remain manual hardware checks; Intel macOS and Linux ARM rows remain explicitly non-publishable.
