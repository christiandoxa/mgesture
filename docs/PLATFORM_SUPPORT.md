...
The CI graph has one Python job and one Mojo job for each of the six targets. Mojo jobs for Intel macOS and Windows use source-contract mode: they validate the canonical source and Python fallback on the native runner without claiming an unsupported native Mojo compiler.
